...
describe('telemetry', () => {
  it('writes separate JSONL streams with byte counts', async () => {
    ...
    await telemetry.model({ timestamp: 'now', task_id: 't', public_tool: 'roblox_inspect', public_operation: 'status', duration_ms: 1, request_bytes: 2, response_bytes: 3, success: true });
    ...
    expect(JSON.parse(await readFile(path.join(root, 'logs', 'model-visible.jsonl'), 'utf8'))).toMatchObject({
      public_tool: 'roblox_inspect', runtime_version: '0.2.0', chrrxs_version: '3.0.3',
    });
    expect(JSON.parse(await readFile(path.join(root, 'logs', 'chrrxs-internal.jsonl'), 'utf8'))).toMatchObject({
      chrrxs_tool: 'get_place_info', runtime_version: '0.2.0', chrrxs_version: '3.0.3',
    });
